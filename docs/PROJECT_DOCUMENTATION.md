...
### 6.3 LLM Provider Routing & Dynamic Token Budget

| Provider | Active Model Chain | Purpose | Default Status |
| :--- | :--- | :--- | :--- |
| **Groq Cloud** | `openai/gpt-oss-20b`, `openai/gpt-oss-120b`, `qwen/qwen3.6-27b` | Ultra-fast cloud inference (500+ tokens/sec) with automated multi-model failover | Primary (First-Class Cloud Gateway) |
| **OpenAI Compatible** | User-configured | External OpenAI-compatible gateway | Secondary |
| **Local Ollama** | `llama3:latest` | Local offline development | Fallback (if no cloud API key provided) |

* **Dynamic Context Token Budget**: ARCHER employs slotted top-4 chunk windowing with character constraints (max 450 characters per excerpt), guaranteeing that prompt payloads stay well within token limits (TPM) while eliminating HTTP 413 errors.
* **Reasoning Token Filtering**: Internal thoughts (`<think>...</think>`) generated by reasoning models are stripped automatically in both backend pipelines and frontend components.
